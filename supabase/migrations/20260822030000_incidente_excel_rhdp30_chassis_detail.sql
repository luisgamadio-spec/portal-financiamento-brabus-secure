-- Fase Excel-RH-DP-3.0 -- adiciona seller_name a operational_salary_details.
--
-- OBJETIVO: implementar as abas 5_CHASSIS_FINANCIADOS/6_TODOS_CHASSIS_
-- VENDEDOR do Excel historico com dados operacionais reais (ver mudanca de
-- frontend correspondente em portal-app.js -- buscarDetalheOperacionalPara
-- Fechamento/chassisDetailRowHistoricoAba5/6), sem criar nenhuma RPC nova:
-- operational_salary_details ja e MASTER-capaz de retornar TODOS os
-- vendedores em uma unica chamada bulk (p_seller_id=null), ja usa o
-- pipeline canonico corrigido (Incidente Salary-Details-Later-Return),
-- ja mascara chassi e ja nao expoe identidade de cliente.
--
-- UNICO GAP: a funcao ja resolve o nome do vendedor internamente em
-- eligible_sellers (u.nome/ps.name, cobrindo tanto usuarios quanto o
-- fallback legado portal_sellers), mas nunca expunha esse campo no JSON
-- de saida -- o unico consumidor anterior (modal "Conferencia segura de
-- vendas", showOperationalSalaryDetails) sempre chamava por vendedor
-- (p_seller_id preenchido) e ja sabia o nome de quem abriu o modal. Uma
-- exportacao MASTER bulk precisa resolver o nome de QUALQUER vendedor
-- devolvido, entao o campo precisa vir da propria RPC.
--
-- MUDANCA: puramente aditiva -- adiciona "ps.name as seller_name" em
-- visible_sales (coluna ja disponivel via eligible_sellers, so nao
-- selecionada), propaga por detail_rows e pelo jsonb_build_object final.
-- ZERO alteracao de JOIN, WHERE, agregacao ou regra financeira -- a
-- reconciliacao vendedor-a-vendedor (66/66, 0 divergentes, 414 vendas,
-- 179 financiamentos) permanece identica antes/depois, retestada apos a
-- promocao.
--
-- CORRECAO DE REGISTRO: a certificacao desta fase revelou que os
-- incidentes anteriores (Salary-Details-Later-Return e Model-Metrics-
-- Later-Return) atribuiram os chassis das sentinelas ROGLECIA e STEFAN
-- trocados entre si (erro de metodologia numa consulta SQL sem ORDER BY
-- explicito, que assumiu incorretamente que a ordem de retorno preservava
-- a ordem da lista WHERE IN). O nome correto, confirmado aqui por join
-- explicito e pelo proprio seller_name retornado pela RPC:
--   93XHTGK1WTCS21537 (GASTAO/SEMINOVOS)  -> STEFAN KIS ALONCO (nao Roglecia)
--   93XHTGK1WVCT35046 (ALPHAVILLE/NOVOS)  -> ROGLECIA SILVA DO NASCIMENTO (nao Stefan)
-- RALPHO (93XGTGK1WVCT35017) e T34702 permanecem corretos como reportado.
-- Este engano foi apenas de rotulagem nos relatorios/comentarios dos
-- incidentes anteriores -- os patches SQL ja promovidos (90143de, d2af4a0)
-- operam inteiramente por chassi/finance_operation, nunca por nome, e
-- portanto nao foram afetados nem precisam ser corrigidos.
