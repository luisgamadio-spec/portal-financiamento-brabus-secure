-- Fase 21.1 -- Hotfix sistemico das funcoes analiticas restantes.
--
-- Contexto (Fase 21.0): Incidente 19.1 corrigiu a resolucao de identidade do
-- proprio vendedor (VENDEDOR/is_seller) apenas em operational_metrics(),
-- trocando o padrao quebrado "ps.portal_user_id = v_user_id" (portal_user_id
-- nunca e populado na populacao real) por match via CPF normalizado
-- (usuarios.cpf_normalizado = portal_sellers.cpf_normalizado, com checagem de
-- unicidade). As demais 6 funcoes analiticas nunca receberam essa correcao e
-- permaneciam quebradas para AUTO-VISUALIZACAO de qualquer vendedor.
--
-- Confirmado ao vivo, antes desta migration: Renan Guedes de Paiva tinha
-- R$ 173.251,87 em producao visivel via operational_metrics(), mas
-- operational_salary_details() (botao DETALHES / Conferencia segura de
-- vendas) retornava rows:[] para a propria identidade dele.
--
-- Esta migration replica EXATAMENTE o mesmo padrao ja homologado (19.1) nas
-- 6 funcoes abaixo. Mudanca cirurgica: apenas a resolucao de identidade
-- dentro de cada eligible_sellers (ou equivalente); nenhuma formula, filtro
-- de periodo, agrupamento ou agregacao foi alterada. Testado extensivamente
-- via funcoes candidatas isoladas (sufixo _candidate_211, removidas ao final)
-- contra: MASTER (5/5 identico byte-a-byte), GERENTE (5/5 identico),
-- DIRETOR (5/5 identico), ANALISTA (5/5 identico), 14 vendedores reais
-- (self-view identico ao MASTER filtrado pelo mesmo seller_id), 2 pares
-- adversariais cross-user (mesma loja e lojas diferentes, zero vazamento),
-- e teste de zero legitimo (periodo sem producao real retorna rows:[] sem
-- erro). Grants e search_path preservados sem alteracao (grants-only nao se
-- aplica aqui pois ha mudanca de corpo de funcao, mas o ACL nao foi tocado
-- pela migration em si -- confirmado igual antes/depois via has_function_privilege).
--
-- Funcoes corrigidas: operational_salary_details, operational_analyst_commission_metrics,
...
--
-- Fora de escopo desta fase (nao tocado): master_convidar_usuario e
-- master_corrigir_revisao_cadastral tambem usam portal_sellers.portal_user_id,
-- mas em semantica DIFERENTE (checagem de posse/conflito de NBS durante
-- convite/correcao administrativa, nao resolucao da identidade do chamador) --
-- confirmado via leitura de codigo na Fase 21.0/21.1, nao sao bugs equivalentes.
--
-- Rollback exato disponivel: pg_get_functiondef() de cada uma das 6 funcoes
-- capturado antes desta promocao (scratchpad da sessao).
...
begin
      ...
      and (
        ...
        or (
          ...
          and (
            ...
            or (v_is_seller and v_caller_cpf is not null and v_caller_cpf <> '' and regexp_replace(coalesce(ps.cpf_normalizado, ''), '\D', '', 'g') = v_caller_cpf and (select count(*) from public.portal_sellers ps2 where regexp_replace(coalesce(ps2.cpf_normalizado, ''), '\D', '', 'g') = v_caller_cpf) = 1)
  ...
  visible_sales as (
    ...
    join eligible_sellers ps on ps.id = s.seller_id
  ...
  join eligible_sellers es on es.id = r.seller_id
...
begin
  ...
  sales_global_latest as (
    select distinct on (s.chassis)
      s.id, s.sale_date, s.chassis, s.seller_id,
...
begin
  ...
  with visible_sales as (
    ...
    select
      ...
      coalesce(ps.name, s.seller_source_name, 'SEM VÍNCULO') as seller_name
        ...
        or (
          ...
          and (
            ...
            or (
              ...
              v_is_seller and v_caller_cpf is not null and v_caller_cpf <> '' and regexp_replace(coalesce(ps.cpf_normalizado, ''), '\D', '', 'g') = v_caller_cpf and (select count(*) from public.portal_sellers ps2 where regexp_replace(coalesce(ps2.cpf_normalizado, ''), '\D', '', 'g') = v_caller_cpf) = 1
            ...
            or (
              ...
              and upper(coalesce(s.store, ps.store, '')) = v_store
...
begin
  ...
  with eligible_sellers as (
    select ps.* from public.portal_sellers ps
     where ps.active
       and upper(trim(coalesce(ps.profile_type,'')))='VENDEDOR'
       and upper(trim(coalesce(ps.status,''))) not in ('REVENDA','INATIVO','MASTER')
       and (v_is_master or (
         (upper(trim(coalesce(ps.status,'')))=any(v_departments)
          or (upper(trim(coalesce(ps.status,'')))='NOVOS/SEMINOVOS'
              and ('NOVOS'=any(v_departments) or 'SEMINOVOS'=any(v_departments))))
         and (v_is_director or (v_is_seller and v_caller_cpf is not null and v_caller_cpf <> '' and regexp_replace(coalesce(ps.cpf_normalizado, ''), '\D', '', 'g') = v_caller_cpf and (select count(*) from public.portal_sellers ps2 where regexp_replace(coalesce(ps2.cpf_normalizado, ''), '\D', '', 'g') = v_caller_cpf) = 1)
              or (not v_is_seller and upper(coalesce(ps.store,''))=v_store))))
  ), sales_latest as (
    select distinct on (s.chassis)
      s.id,s.sale_date,s.chassis,s.seller_id,es.name as seller_name,
      coalesce(nullif(s.store,''),es.store,'SEM LOJA') as store,
      s.department,coalesce(nullif(s.vehicle_model,''),'NÃO INFORMADO') as vehicle_model,
      s.sale_value
    from public.portal_sales s join eligible_sellers es on es.id=s.seller_id
    order by s.chassis,s.sale_date desc,s.id desc
  )
  select coalesce(jsonb_agg(jsonb_build_object(
