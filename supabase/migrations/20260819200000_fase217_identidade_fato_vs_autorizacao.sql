-- Fase 21.7 — Separacao definitiva entre identidade do fato (seller_user_id)
-- e autorizacao de acesso (usuarios.ativo).
--
-- Em 7 funcoes analiticas, a CTE eligible_sellers exigia usuarios.ativo=true
-- para que uma pessoa pudesse ser reconhecida como dona de um fato (venda/
-- financiamento). Isso confundia duas perguntas distintas: DE QUEM E este
-- fato (seller_user_id, identidade) versus ESTA PESSOA PODE ACESSAR O PORTAL
-- AGORA (usuarios.ativo, autorizacao). Um vendedor recem-cadastrado, ainda
-- pendente de primeiro acesso (ativo=false), ou um vendedor posteriormente
-- inativado, podia ter vendas corretamente reconciliadas via seller_user_id
-- (Fase 21.6B) e mesmo assim desaparecer das analises de MASTER/GERENTE/
-- ANALISTA/DIRETOR simplesmente por nao estar ativo.
--
-- Correcao: eligible_sellers deixa de exigir u.ativo=true no ramo usuarios.
-- O gate de AUTORIZACAO DO CHAMADOR (quem esta chamando a funcao agora, via
-- auth.uid()) NAO foi tocado em nenhuma das 8 funcoes — usuario inativo
-- continua em default-deny para self-view, exatamente como antes.
--
-- O fallback legado portal_sellers (NOT EXISTS ... u2.ativo=true) foi
-- deliberadamente PRESERVADO sem alteracao: uma tentativa inicial de
-- remover o ativo=true tambem dali causou uma REGRESSAO comprovada em teste
-- populacional (queda de 11304 para 10867 linhas em operational_salary_details
-- num periodo amplo) — fatos ainda nao reconciliados via seller_user_id, que
-- dependem do fallback portal_sellers para pessoas inativas, ficavam orfaos.
-- Revertido antes da promocao; NOT EXISTS mantem o comportamento historico.
--
-- operational_reporting_summary recebeu tratamento diferente: nao possui
-- eligible_sellers, mas usava 'left join usuarios u on u.id=s.seller_user_id
-- and u.ativo=true' apenas para enriquecer nome/loja de exibicao — removido
-- o and u.ativo=true dali (identidade nao depende de ativo).
--
-- Testado via fixture sintetica (usuario/venda descartados, nunca
-- persistidos) nos 3 estados PENDENTE/ATIVO/INATIVADO: MASTER sempre ve o
-- fato com seller_user_id identico nos 3 estados; self-view permanece negado
-- (42501) em PENDENTE e INATIVADO, e so mostra o proprio fato quando ATIVO.
--
-- Auditoria da populacao real (6 usuarios inativos hoje, excluindo conta de
-- teste): NENHUM possui fato reconciliado via seller_user_id — todos, se tem
-- fatos, ainda dependem exclusivamente do fallback portal_sellers legado
-- (inalterado por esta migration). Ou seja: 0 fatos adicionais aparecem hoje
-- para Alexandre/Cristina/Fabio/Barbara/Rui Luis; esta e uma correcao
-- estrutural preventiva, comprovada via fixture, nao uma recuperacao de
-- dados hoje visivelmente ausentes. Fix financeiro 21.6C.1 (latest VALIDATED
-- batch) preservado integralmente — corpos desta migration foram extraidos
-- FRESH de producao pos-21.6C.1, nao de migration antiga.
